...
class Settings(BaseSettings):
    ...
    rescan_delay_seconds: int = Field(
        default=300,
        description="Delay antes de ejecutar rescan (5 minutos default)"
    )
    rescan_timeout_hours: int = Field(
        default=72,
        description="Timeout para remediación sin verificar (PEN-001)"
    )
    ...
    speed_bonus_threshold_hours: int = Field(
        default=24,
        description="Umbral para bonus de velocidad (PTS-004)"
    )
    speed_bonus_multiplier: float = Field(
        default=1.5,
        description="Multiplicador del bonus de velocidad"
    )
    ...
    timeout_check_interval_minutes: int = Field(
        default=60,
        description="Intervalo para verificar timeouts (cada hora)"
    )
