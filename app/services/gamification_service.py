...
class GamificationService:
    ...
    async def get_leaderboard(
        self, limit: int = 10, team_id: str | None = None, timeframe: str | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    async def get_user_badges(self, user_id: str, limit: int = 50) -> list[dict[str, Any]]:
        ...
        badges = (
            await self.awards.find({'user_id': user_id})
            .sort('awarded_at', -1)
            .limit(limit)
    ...
    async def get_user_stats(self, user_id: str) -> dict[str, Any]:
        ...
        recent_txns = (
            await self.point_txns.find({'user_id': user_id})
            .sort('timestamp', -1)
            .limit(5)
            .to_list(length=5)
        )
        ...
        stats = (
            points_stats[0]
            if points_stats
            else {'total_earned': 0, 'total_lost': 0, 'max_single_gain': 0}
        )
        stats.pop('_id', None)
        ...
        return {
            'user_id': user_id,
            'balance': balance,
            'badges': {'total_count': len(badges), 'by_tier': badge_tiers, 'recent': badges[:5]},
            'points_stats': stats,
            'recent_transactions': recent_txns,
        }

    async def get_recent_activity(
        self, limit: int = 20, user_id: str | None = None, team_id: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        # Transacciones
        txns = (
            await self.point_txns.find(query)
            .sort('timestamp', -1)
            .limit(limit // 2)
